...
- chore(deps-dev): bump eslint-plugin-react-refresh from 0.5.3 to 0.5.5 ([#123](https://github.com/djryanj/media-tagger/pull/123))
...
- Automatic downloads. Files are now only saved from the download manager, so nothing is lost behind a browser prompt that stacked up or was dismissed unseen, and nothing is downloaded that the user did not ask for. The submit buttons no longer download either, and are renamed accordingly: `Tag all and download` is now `Tag all files`, and the per-file `Tag and download` is now `Tag file`.
...
- chore(deps): bump docker/login-action from 4.4.0 to 4.6.0 ([#106](https://github.com/djryanj/media-tagger/pull/106))
- chore(deps): bump renovatebot/github-action from 46.1.21 to 46.2.4 ([#107](https://github.com/djryanj/media-tagger/pull/107))
...
- chore(deps): bump dorny/paths-filter from 4.0.1 to 4.0.2 ([#88](https://github.com/djryanj/media-tagger/pull/88))
...
- Fixed a lint error (`react-hooks/set-state-in-effect`) not present before caused by new ESLint rules by scheduling setState in a microtask inside the preview URL effect. This prevents cascading renders and aligns with stricter React Hooks linting in CI. [#45](https://github.com/djryanj/media-tagger/pull/45)
...
- Individual tagging mode now supports per-file `Tag and download` actions that process one file, remove it from the queue on success, and notify the user that it will not be included in the later bulk run. [#38](https://github.com/djryanj/media-tagger/issues/38)

### Fixed

- Long filenames in preview cards no longer break the tagging layout, and automated coverage now exercises shared preview removal plus the new per-file tagging workflow. [#38](https://github.com/djryanj/media-tagger/issues/38)
...
- chore(deps): bump actions/github-script from 8.0.0 to 9.0.0 ([#20](https://github.com/djryanj/media-tagger/pull/20))
- chore(deps): bump pnpm/action-setup from 5 to 6 ([#21](https://github.com/djryanj/media-tagger/pull/21))
- chore(deps): bump renovatebot/github-action from 46.1.8 to 46.1.10 ([#22](https://github.com/djryanj/media-tagger/pull/22))
